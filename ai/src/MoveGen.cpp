...
void AdjacentMoveList::Iterator::Advance() {
    while (current_vector != std::end(kAdjacent)) {
        ...
        GridPosition new_grid_pos =
            move_list->grid_pos + cell_side_vector.second;
        if (InBounds(new_grid_pos) &&
            !HasWall(move_list->pos, move_list->grid_pos, cell_side_vector)) {
            ret = new_grid_pos;
            return;
        }
    }
...
PawnMoveList::PawnMoveList(const Position& pos)
    : MoveListCommon(pos),
      player(pos.GetPawnPosition(pos.GetCurrentTurn())),
      other(pos.GetPawnPosition(pos.GetCurrentTurn() == kWhite ? kBlack
                                                               : kWhite)),
      adjacent(player, pos),
...
void PawnMoveList::Iterator::NextAdjacent() {
    while (current != move_list->adjacent.end()) {
        ...
        if (std::ranges::find(move_list->jump_moves, jump) !=
            move_list->jump_moves.end()) {
            straight_jump = true;
        }
    }
    if (!will_jump) {
